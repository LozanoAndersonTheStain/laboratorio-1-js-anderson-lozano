...
const turnoComputadora = (puntosMinimos) => {
  ...
  setTimeout(() => {
    if (puntosMinimos === puntosComputadora)
      alert('Has empatado con la computadora')
    else if (puntosMinimos > 21) alert('La computadora te ha ganado')
    else if (puntosComputadora > 21)
      alert('Genial, le has ganado a la computadora')
...
btnPedir.addEventListener('click', () => {
  ...
  if (puntosJugador > 21) {
    ...
    console.warn('sorry manito, has perdido')
    turnoComputadora(puntosJugador)
  } else if (puntosJugador === 21) {
    btnPedir.disabled = true
    btnDetener.disabled = true
    console('Genial tío, le habeis ganado a la maquina')
    turnoComputadora(puntosJugador)
  }
})
